...
fn roundtrip(nstreams: u64, nmessages: usize, data: Bytes, send_all: bool) {
    ...
    let server = Connection::new(e1, server.aio(), Config::default(), Mode::Server)
        .expect("connection")
        .map_err(|()| panic!("connection closed"))
        .for_each(|stream| {
    ...
    let client = future::lazy(move || {
        ...
        let c = Connection::new(e2.clone(), client.aio(), Config::default(), Mode::Client)
            .expect("connection");
