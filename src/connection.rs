...
use crate::{
    ...
    frame::{
        codec::FrameCodec,
        header::{self, Header, Type, ACK, ECODE_INTERNAL, ECODE_PROTO, FIN, RST, SYN},
        Data, Frame, GoAway, Ping, RawFrame, WindowUpdate,
    },
    notify::Notifier,
    ...
    T: AsyncRead + AsyncWrite,
{
    ...
    pub fn poll(&self) -> Poll<Option<StreamHandle<T>>, ConnectionError> {
        ...
        while let Some(id) = connection.incoming.pop_front() {
            let stream = if let Some(stream) = connection.streams.get(&id) {
                debug!(
                    "{}: {}: incoming stream of {:?}",
                    connection.id, id, *connection
                );
    ...
    T: AsyncRead + AsyncWrite,
{
    ...
    fn flush_pending(&mut self) -> Poll<(), ConnectionError> {
        ...
        while let Some(frame) = self.pending.pop_front() {
            ...
            if let AsyncSink::NotReady(frame) =
                self.resource.start_send_notify(frame, &self.tasks, 0)?
            {
                if self.pending.len() >= self.config.max_pending_frames {
                    return Err(ConnectionError::TooManyPendingFrames);
                }
        ...
        if self
            .resource
            .poll_flush_notify(&self.tasks, 0)?
            .is_not_ready()
        {
            return Ok(Async::NotReady);
        }
            ...
            match self.resource.poll_stream_notify(&self.tasks, 0)? {
                Async::Ready(Some(frame)) => {
                    ...
                    let response = match frame.dyn_type() {
                        Type::Data => self.on_data(Frame::assert(frame))?.map(Frame::into_raw),
                        Type::WindowUpdate => self
                            .on_window_update(&Frame::assert(frame))?
                            .map(Frame::into_raw),
                        Type::Ping => self.on_ping(&Frame::assert(frame)).map(Frame::into_raw),
                        Type::GoAway => {
                            self.kill();
                            return Ok(Async::Ready(()));
                        }
    ...
    fn on_data(&mut self, frame: Frame<Data>) -> Result<Option<Frame<GoAway>>, ConnectionError> {
        ...
        if frame.header().flags().contains(SYN) {
            ...
            if frame.body().len() > DEFAULT_CREDIT as usize {
                error!(
                    "{}: {}: initial data for stream exceeds default credit",
                    self.id, stream_id
                );
                return Ok(Some(Frame::go_away(ECODE_PROTO)));
            }
        ...
        let frame = if let Some(stream) = self.streams.get_mut(&stream_id) {
    ...
    ) -> Result<Option<Frame<GoAway>>, ConnectionError> {
        let stream_id = frame.header().id();
    ...
    fn on_ping(&mut self, frame: &Frame<Ping>) -> Option<Frame<Ping>> {
        ...
        debug!(
            "{}: {}: received ping for unknown stream",
            self.id, stream_id
        );
        None
    }
    ...
    fn finish(&mut self, id: stream::Id) -> Result<(), ConnectionError> {
        let frame = if let Some(stream) = self.streams.get_mut(&id) {
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        loop {
            ...
            if inner.config.window_update_mode == WindowUpdateMode::OnRead {
                let inner = &mut *inner;
                let frame = if let Some(stream) = inner.streams.get_mut(&self.id) {
                    if stream.window == 0 {
